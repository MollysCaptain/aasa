...
| **Relevance cutoff is a single global threshold** | Retrieval now drops chunks past `RELEVANCE_THRESHOLD` (0.52) so an unrelated query no longer claims "15 real deployments matched". One global number can't perfectly separate every absurd query from every real one — in calibration, 1 of 8 nonsense queries still had a single coincidentally-close chunk. **Joint call (Ash + Gabi, 2026-07-27):** with 3,023 real cases behind it and retrieval currently built only from two fixed dropdowns, there is effectively no way to submit a genuinely irrelevant query through the shipped UI — so the threshold is defence-in-depth, not a fix for an observed problem. | Per-industry or adaptive thresholds; or surface the distance to the user. Re-derive with `tests/distancecheck.py` if the embedding model or distance metric changes — **and if free text is ever added to the UI (Gabi's condition), re-run the nonsense-control sweep before shipping it**, since that's the first time an actually irrelevant query becomes reachable. |
