...
| **The summary can fall back to deterministic prose** | Found by a tester 2026-07-28: when the recommended stack contained no seat-priced tool, the model returned empty content and the Summary tab and export both rendered blank. Root cause was a prompt-shape mismatch (the few-shot examples showed a prose cost line while runtime passed a raw Python dict including `'assistant': None`). Fixed, and the summary path now **cannot** render empty — if the model returns nothing, a factual sentence is assembled from the already-computed stack, costs and budget. | The fallback is a safety net, not a feature: `summary_fallback_used` is logged to telemetry so if it starts firing regularly that signals the prompt needs work rather than letting the net carry the product. |
...
| **The repo redistributes the source dataset, having said it wouldn't** | Deploying to Streamlit Community Cloud required committing `chroma_store/` (2026-07-28, Build Guide 36) — Cloud only gets what's in the repo and can't rebuild the store there. The store isn't just vectors: its chunk metadata carries the upstream titles, organisations, outcomes and ~3.1 MB of verbatim case prose. The raw CSV is still excluded, but the earlier framing — "the upstream data isn't distributed via this repo" — stopped being true. | Permitted under MIT **with attribution**, now recorded in `docs/data-attribution.md` and referenced from the README, `.gitignore` and Build Guide 13. A cleaner long-term fix is a store built from embeddings plus minimal metadata (ids and links, no prose), so the app works without redistributing text — but that changes what Block C can display and wasn't a freeze-week change. |
...
| **43% of selectable input combinations have no evidence behind them** | The two dropdowns allow 432 workflow × industry combinations, but the corpus only populates 247 of them — **185 pairs have zero real cases** (e.g. there is no "Procurement in Education" deployment in the library at all). The lists are each derived from real corpus values independently, so every *value* is real while many *pairings* are empty. Retrieval is semantic, so those queries still return the nearest cases from adjacent industries. Full sweep 2026-07-27 found 205/47%; recounted 2026-07-30 against the rebuilt store, 185/43%. The count moves whenever the store is rebuilt — `python tests/distancecheck.py --full` prints the live figure. | Now **disclosed in the product**: the banner distinguishes real matches from nearest-comparable ones instead of claiming "N real X Y deployments matched" (which was false for these 185). Proper fix is to disable or flag unpopulated pairs in the dropdowns before the user submits — deferred, tracked for the next iteration. |

## Validation limitations

| Limitation | Why it exists | Roadmap fix |
|---|---|---|
| **Small-sample validation (8 participants / 12 sessions)** | Realistic for a 2-person, 4-week team. | Larger N post-capstone. Rates are reported **with 90% credible intervals** (Card P.14: net value 100%, CI 72–99%; export rate 83%, CI 59–93%), and we make **no comparative claim** ("A beat B") because that needs a properly powered study with a control group. At n=8 a single changed answer moves a rate by 12.5 points — read the intervals, not the point estimates. |
| **Participants were all professional contacts of the team** | Convenience sampling: 8 real practitioners at 7 real companies, but every one recruited through our own networks, and two work at the same employer. | This is the most likely source of inflated trust scores and we have no way to correct for it retrospectively. A neutral round would recruit participants with no relationship to the team. Disclosed here and in the P.14 write-up rather than left for a marker to infer. |
| **Telemetry has no user identifier, so "sessions" ≠ distinct people** | Deliberate privacy choice — one person can submit the form several times in a sitting, and several participants generated more than one blueprint. | The 8 survey responses are the reliable participant count, and for this round they are independently corroborated against the recorded session sheet (`AI-Assisted Stack Architect User Testing (pseudonymised).xlsx` — only the pseudonymised copy is committed; the unredacted original is gitignored because it holds participants' names and employers) participant by participant, in order. |
| **The trust-score improvement is not a measured before/after** | The earlier round scored median 3/5 and missed the ≥4/5 target; the final round scored 5/5 and met it. But the build changed *and* the participants changed between rounds, so the two effects cannot be separated. | We state the result without claiming causation. "Our fixes raised trust from 3 to 5" would need the same people testing both builds, which did not happen. The earlier round's numbers are kept in the P.14 write-up rather than deleted. |
...
| **The 100% compliance figure covers 2 recorded profiles, not the 5 live regulated sessions** | 5 of the 8 participants chose the regulated posture, so real users exercised the compliance path far more than our own dry run did — but those live runs were not individually checked against the `GOVERNABLE_FOR_REGULATED` allowlist. | Log the filter's decisions per run so live sessions can be audited after the fact, rather than only re-runnable profiles. Until then the 100% claim is scoped to the two recorded profiles, and says so wherever it appears. |
