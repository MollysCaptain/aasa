...
1. **List every limitation you already know about** — you don't need to discover new ones, you need to write down what you've already learned while building:
   ...
   - Validation sample size is 8 real testers, not a statistically representative study.
   - All four validation targets, including trust score, were met as of the final testing round (2026-07-28). State this factually, but do not present it as a measured improvement — the difference from earlier interim numbers is more plausibly small-sample noise than anything the team changed.
   ...
   - 5 of 8 real testers chose the Regulated privacy posture, so live sessions exercised the compliance filter far more heavily than the 2-profile dry run (P.9) did. Those live runs weren't individually audited against `GOVERNABLE_FOR_REGULATED`, so the 100% compliance-rule pass rate is explicitly scoped to the two recorded dry-run profiles, not the 5 live regulated sessions.
   - One session generated, exported, and downloaded a PDF without submitting the trust survey — it's counted in the 12 `results_shown` / 10 `export_clicked` events, not in the 8 survey responses.
   - 5 of the 8 real testers are non-native English speakers — this closes Card P.13's ask for testing with real participants rather than simulated personas.
   - Telemetry (Card 3.3) has no user/session identifier — session counts in `data/telemetry.log` can't be mapped to distinct testers, so "12 completed sessions" and "8 survey responses" aren't necessarily 12 and 8 different people.
   - ~~Retrieval (Card 2.2) has no relevance threshold~~ **Fixed 2026-07-27** — `app/pipeline.py` now applies `RELEVANCE_THRESHOLD = 0.52`, calibrated against real measured distances (`tests/distancecheck.py`), so an obscure workflow/industry combo returns few-to-no matches instead of always claiming "15 real deployments matched." Residual gap: 205 of the 432 selectable workflow × industry combinations have no underlying cases at all — the threshold surfaces that honestly, it doesn't fix the sparse coverage itself.
   - The `project_name` field (Icebox B.5, `app/intake.py`) is a free-text input the UI asks users not to put PII into but doesn't enforce. **Corrected 2026-07-28** — an earlier version of this card claimed this value could leak into `data/aasa-saved-blueprints.json` because that file is git-tracked; that claim was false and is retracted. Saves only ever go through `st.session_state` and a client-side `st.download_button`, never a runtime write to `data/` — confirmed empirically during real testing (31 `blueprint_saved` events, none written to any tracked file). The committed JSON is a stale placeholder from the team's own earlier testing, not live user data. The narrower, real residual limitation: the field is still unvalidated, so a user could type something identifying into it, and it would appear on-screen and in that session's exported download.
   - ~~Card P.11 (real user testing) may still be "In Progress"~~ **Complete 2026-07-28.**
...
| Small sample validation (8 testers) | Realistic for a 2-person, 4-week team | Larger N in a post-capstone iteration. Rates are reported with credible intervals (Card P.14), and no comparative ("A beat B") claim is made, since that would require a properly powered study this team size/timeline can't run. |
| All 8 testers came from the team's own professional networks | Convenience sampling — plausibly inflates trust scores and other self-reported metrics | State plainly; recruit an independent panel in a post-capstone iteration |
| Regulated-filter compliance (100% pass) is scoped to 2 dry-run profiles, not all 5 live regulated sessions | Live sessions weren't individually audited against `GOVERNABLE_FOR_REGULATED` | Add automated per-session compliance logging so live sessions can be audited too |
| One session exported a PDF without completing the trust survey | Export and survey submission aren't tied to the same required step | Counted separately (12 `results_shown`/10 `export_clicked` vs. 8 survey responses) — don't conflate the two when reporting |
...
| ~~Retrieval has no relevance threshold~~ — Fixed 2026-07-27 | `RELEVANCE_THRESHOLD = 0.52` now filters low-relevance matches (`app/pipeline.py`), calibrated via `tests/distancecheck.py` | Residual: 205 of 432 workflow × industry combinations still have zero underlying cases — a coverage gap, not a threshold problem |
| `project_name` free-text field is unvalidated | Icebox B.5 added a `st.text_input` with a PII warning but no enforcement. Corrected 2026-07-28: it does not write to any tracked file — see step 1 above for the retracted claim | Add a lightweight scrub/warning before the value is shown on-screen or exported |
| Card P.11 (real user testing) | Complete 2026-07-28 — 8 real testers, final round | No action needed |
```
