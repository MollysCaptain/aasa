...
- **Mainstream Validation Overlay:** population-level Stack Overflow survey benchmarks as supporting context (e.g. "X% of similarly-sized companies report using Y") — explicitly labelled as population-level, not case-specific, since the case library has no org-size join key. ~~The concrete asset for this already exists: the colleague's `stackpunk`/`Gabi` branch built `data/technology_landscape.csv`~~ **Correction (2026-07-19, B.4 feasibility gate):** that file and its `extract_tech_landscape.py` script are **not in the repo on any branch** — the earlier claim (via doc 19) described work that was never committed. The asset must be obtained from Gabi directly (or rebuilt from a fresh survey download) before this item is actionable — see the gate results in `24…30`-series Build Guide 29. Whoever picks this up will still need to build a mapping layer between the survey's own Industry/OrgSize categories and this app's taxonomy before joining it to case-library output.
