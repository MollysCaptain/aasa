...
The rebuilt prototype (`aasa-prototype.html`) is a static visualization mockup, not the real product build — see the clarification above. It demonstrates the same five-constraint flow against its smaller curated slice: pick five constraints → it retrieves matched deployments from that slice, ranks their tools, estimates a seat+token cost against your budget, and links every recommendation back to its source. The actual product (the Streamlit app in `app/`) does this against the full 3,023-case dataset via the real Chroma-backed pipeline in `app/pipeline.py`.
